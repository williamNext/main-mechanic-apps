...
| **3. Admin Management** | Create/deactivate/reactivate mechanic, dashboard, lists, details, financial report, `admin` app rewired and fully off Supabase | ✅ **Complete** (2026-08-15) — ticket 12 (e2e), `41d7cd8` |
...
| **4. Shared theme and model types** | Extract `@main-mechanic/theme` and `@main-mechanic/types`; rename `timeslotId` → `timeSlotId`; every client type derives from the server instead of hand-mirroring it. Discharges D-AH | ✅ **Complete** (2026-08-22) — tickets 01–13, theme track `47b1a3c`…`9a69ca2`, rename `4c25317`, types track `b9e0f68`…`02d1b47`. See [`SPEC-phase-4-shared-theme-and-models.md`](SPEC-phase-4-shared-theme-and-models.md) and [ADR 0003](docs/adr/0003-client-types-derive-from-the-server.md) |
...
| **D-AI** | Phase 4 shape | **One phase, two independent tracks (theme, types), with the `timeSlotId` rename between them.** | Both tracks were deferred from Phase 3b under D-AH. The rename sits between them so the theme track isn't blocked behind the phase's only run-time-risk ticket, and so `server/src/api-types.ts` (the types track's first ticket) names the post-rename spelling instead of needing its own rewrite. |
| **D-AJ** | Client type authority | **Hybrid: wire and request types are derived from the server; view models are hand-written in `@main-mechanic/types`.** | A wire or request type is only correct if it matches a validator or serializer the server already enforces, so deriving it removes the possibility of drift. A view model (`User`, `Mechanic`, `AdminUser`, `PaginatedResult<T>`, `CompleteAppointmentInput`, `AdminFilters`) has no server equivalent — it exists because a screen needs a shape the wire never sends. See `CONTEXT.md`'s four-kind glossary. |
| **D-AK** | `admin` and the shared theme | **`admin` consumes `@main-mechanic/theme` — a soft rule.** | Its only live token read is one background color on one navigation layout; its own login screen paints over it, and its 219 hard-coded hexes are a separate project (debt #1). If consuming the package breaks anything in `admin`, the decision is revised toward `admin` leaving the theme system entirely — that revision is itself a decision row, not a silent patch. |
| **D-AL** | Canonical `letterSpacing` | **`oficina`'s values win** (`headlineXl -0.96`, `headlineLg -0.32`) over `mechanic`'s and `admin`'s `0`. | `DESIGN_GUIDE.md` §3.4 already documented `oficina`'s values as canonical before this phase; the package adopts what the document already called correct rather than averaging the drift. |
| **D-AM** | Design debt scope this phase | **Only debt #7 is discharged. #16 closes as a side effect of D-AO. Every other listed debt is explicitly declined.** | #7 (`success` aliased to a red) is fixed by the new green (D-AX). #16 (Expo-template leftovers) closes because D-AO deletes `themed-text.tsx`, `themed-view.tsx`, `collapsible.tsx` and `icon-symbol.tsx` outright. #1, #2, #4, #5, #6, #8 are out of scope — see `DESIGN_GUIDE.md` §15. |
| **D-AN** | Package split | **Two packages, `@main-mechanic/theme` and `@main-mechanic/types`, split by ownership and audience, not by dependency.** | `theme` is design-owned — a designer changes it, correctness is judged by eye. `types` is server-derived — a server change dictates it, correctness is judged by the compiler. Merging them would put the design system's release cadence on the wire contract's, and would stop a non-React consumer (`server`, `tests-e2e`) from taking the types alone. |
| **D-AO** | What travels into the theme package | **Only `useAppTheme`. 22 scaffolding files are deleted, not moved.** | It is the sole live accessor and the sole place the light/dark decision (debt #4) is made. Deleted per app: `hooks/use-theme-color.ts`, `use-color-scheme.ts`, `use-color-scheme.web.ts`, `components/themed-text.tsx`, `themed-view.tsx`, `ui/collapsible.tsx`, `ui/icon-symbol.tsx`, `ui/icon-symbol.ios.tsx` (8 in `oficina`, 8 in `mechanic`, 3 in `admin`, which has none of the five components), plus the three `hooks/use-theme.ts` copies the package replaces = 22. `use-color-scheme.web.ts` is a platform-resolved file Metro handles but `tsc` never checks — deleting it is smaller and safer than putting an unchecked file into the type surface. |
| **D-AP** | The server's client-facing surface | **One module, `server/src/api-types.ts`, names every response shape and request body a client consumes.** | The types package imports from this module and nowhere else inside `server/`. Several shapes already existed as unexported local types (the timeslot response, the profile response); naming them is mostly promotion, not invention. |
| **D-AQ** | Re-exports this phase creates | **None.** | A surviving re-export file is an invitation to add one app-only token to it, which is how the drift Phase 3b's ticket 06 ended would restart. `oficina/config/env.ts`'s existing re-export stands (D-AD) — this rule is scoped to what Phase 4 creates, not applied retroactively. |
| **D-AR** | Request types as a kind | **A request type is distinct from a wire type, though both are derived from the server.** | A wrong request type is a run-time rejection — the `.strict()` validator refuses it. A wrong wire type is a field that silently reads `undefined`. Concretely, `ServiceItemInput` (request) is not the wire `ServiceItem`: the wire item carries required `id`/`sortOrder`; the completion validator's item is `.strict()` with only `description`/`amountCents`. |
| **D-AS** | The `timeslotId` → `timeSlotId` rename | **`timeSlotId` is canonical, and the rename is full across the platform, not wire-only.** | A wire-only alias would keep two spellings in the codebase and add a translation layer whose only job is hiding them. Scope: every `timeslotId` identifier under `server/src`, `server/scripts`, `server/tests`, plus both consuming apps and the e2e spec — 117 occurrences on 112 lines, one commit. The Drizzle property renames; the SQL column keeps its `'timeslot_id'` name, so no migration runs. `admin` needed no change — its endpoints already emitted `timeSlotId`. |
| **D-AT** | Nullability notation for derived types | **The derived type is the server's named type. No key is optionalised by hand.** | Where the server sends `T \| null`, the client type is `T \| null` — absent means the key is missing, null means known-empty. Mechanical on purpose: the six derived response types carry 81 fields between them, 39 nullable, each otherwise a judgement call made by whoever executes the ticket. |
| **D-AU** | End-to-end gate cadence | **Runs by hand at three points (tickets 05, 06, 12), not per ticket.** | The Playwright suite needs a running server and a browser, which CI does not provide here. Running it after every ticket would slow the loop without adding signal on type-only tickets; running it at the three points carrying real behavior risk (scaffolding deletion, the run-time rename, the final types-file deletion) catches what a green typecheck cannot. |
| **D-AV** | Packages CI job | **Generalised (`npm run --if-present -w ./packages <script>`), not duplicated per package.** | `packages-checks` was hard-coded to `@main-mechanic/wire-client` alone; adding a package under `packages/**` without generalising would trigger the job without checking it — a green run proving nothing. `npm run --workspaces` was rejected: no glob scoping, would also run the three apps and `tests-e2e`. |
| **D-AW** | `Colors.dark` | **Moves into the package byte-identical to `Colors.light`, and stays recorded as debt.** | It is debt #4, out of scope by D-AM. Moving it is not fixing it — `useAppTheme` still hard-locks `'light'` while `app.json` advertises `"automatic"`. |
| **D-AX** | The `success` token | **A new green enters the raw palette; `Colors.success` points at it instead of `colors.secondary` (a red).** | Discharges debt #7. This and the two `letterSpacing` values (D-AL) are the only token *values* this phase changes — no other pixel moves. |
| **D-AY** | ADR scope | **One ADR only, for the type-derivation decision — not for the theme extraction.** | The types track qualifies on all three of this repo's ADR criteria: hard to reverse once three apps and a server share a named surface, surprising to a reader who expects a client to own its own types, and it had real rejected alternatives. The theme package repeats [ADR 0002](docs/adr/0002-npm-workspaces-shared-wire-client.md)'s reasoning with different files and is easy to reverse — copy one file back into three apps. |
| **D-AZ** | Request type derivation | **Derive from `z.input`, never `z.output`/`z.infer`.** | `z.output` is what the handler receives after defaults, coercions and transforms; a client constructs the input, before those apply. The admin filter schema coerces query strings to numbers and defaults `page`/`pageSize`; the booking/completion bodies' nullable-string helper has input `string \| undefined` but output `string \| null`. Deriving from the output would force clients to send explicit `null`s they currently omit and supply values the server already defaults. |
| **D-BA** | Where inference happens | **All `z.input`/`ReturnType` resolution happens inside `api-types.ts`. `@main-mechanic/types` contains no inference expression and declares no `zod`/`drizzle-orm` dependency.** | Measured, not assumed: with every alias resolved server-side, an app's `tsc --noEmit --listFiles` still loads `drizzle-orm`'s and `zod`'s declaration files out of `server/node_modules`, because type aliases are lazy and no build step creates a `.d.ts` boundary (D-AA forbids one) — verified by probe in ticket 07, exit 0. What this decision actually buys is authorship containment, not graph isolation: no source in the types package a zod major bump can break, and no place a client author is invited to write validator code. See [ADR 0003](docs/adr/0003-client-types-derive-from-the-server.md). |
| **D-BB** | `AdminFilters` vs `AdminFilterQuery` | **They split: `AdminFilters` is a view model (fully-defaulted client state), `AdminFilterQuery` is a request type (server validator, all fields optional).** | `admin` holds `AdminFilters` with `from`/`to` required because the client already applied its defaults; the server's validator makes both optional because it applies its own. Deriving the view model from the validator instead would make `filters.from` `string \| undefined` and break every consumer. |
| **D-BC** | Package name | **`@main-mechanic/types`, not `@main-mechanic/models`.** | Matches the four-kind vocabulary in `CONTEXT.md` (wire type, request type, view model, app-local type) — "model" implies ownership the client half of the package does not have. |
| **D-BD** | CI reach of a `server/**` change | **Each of the three app jobs' `if:` in `ci.yml` includes `needs.changes.outputs.server == 'true'`, so a server-only commit re-runs all three app typechecks.** | Before this phase, `ci.yml` defined a `server` path filter no app job referenced — a server-only commit changing a serializer's return shape ran zero app typechecks, and this phase's derivation would have broken silently one level below hand-mirroring. Same shape of change Phase 3b made for `packages/**`. |
| **D-BE** | The timeslot-list query | **Stays hand-validated and hand-built; no type is invented for it.** | It has no zod schema today; writing one would change which error the endpoint returns for a malformed query, and this phase changes no endpoint behavior. Both consumer apps build the query as a string, not an object, so a hand-written type would have no adoption site without a query-builder refactor this phase does not carry. Recorded as the one request surface the phase leaves open. |
| **D-BF** | Auth request bodies vs the profile body | **`LoginInput`/`RegisterInput` are out of scope — positional service signatures. `UpdateProfileInput` is in scope.** | Every app's auth service takes positional parameters (`login(email, password)`; `signUp(name, email, password)` in `oficina` only), so adopting the derived types would mean a services refactor this phase does not carry — their validators are exported by ticket 07 regardless, for a later phase. `UpdateProfileInput`'s consumers were already object-shaped (`Pick<User, 'name'>` in `oficina`, `Pick<Mechanic, 'name' \| 'specialty'>` in `mechanic`), so tickets 09 and 10 adopt it as a type-annotation swap with no signature change. |
...
- ~~Three separate git repos and three private wire-client stacks~~ **Resolved by the monorepo and
  ...
  `@main-mechanic/wire-client`.
- ~~`constants/theme.ts` and `types/models.ts` remained copy-pasted, three times each~~ **Resolved
  by Phase 4 (D-AH).** `@main-mechanic/theme` and `@main-mechanic/types` now each have one
  implementation; no app declares a design token or a type describing a server response. The one
  hand-mirror Phase 4 deliberately left behind: every app's auth service still takes positional
  parameters (`login(email, password)`; `signUp(name, email, password)` in `oficina` only) instead
  of the derived `LoginInput`/`RegisterInput` request types, because adopting them means changing
  service signatures and call sites — a services refactor, not a type extraction (`D-BF`). Their
  validators are exported from `server/src/api-types.ts` regardless, so a later phase can adopt
